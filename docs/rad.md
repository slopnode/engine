...
3. Resolve materials. Albedo and emission textures load as CPU Images. Sampling uses the same planar UV rules as the game (texel-size / pixels-per-meter, texture size, uv-shift, optional UV lock). Lighting directions use flat face normals only--no normal maps.
