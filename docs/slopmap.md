...
Clip (Shift+X / Edit -> Clip) picks two points on a selected brush face; the cut plane is perpendicular to that face through those points and splits the selected brush(es). F cycles keep Front / Back / Both, Shift+F flips the plane, Enter commits, Esc cancels. Clip does not reject non-convex halves. After clip (and face/edge/vert confirms), verts that share a grid cell with a neighbor brush weld to that neighbor, and exact overlaps merge—no separate weld threshold; off-grid verts (e.g. cylinder rings) are left alone. Punch-out subtracts from brushes using selected faces when you need openings without rebuilding the solid by hand.

In Select mode, Edge and Vert scopes pick brush edges or vertices (screen-space, with click-cycle through overlaps). G translates the selection with free/XYZ drag and grid snap like Brush move: coincident verts on the brush (near-exact position) move together. Intermediate non-convex shapes are allowed while editing; maps save and reload them. Edit → Validate Brushes lists brushes that fail convexity checks (id + reason) so you can select and repair them. R does not rotate edges or verts. Delete removes the parent brush(es), same as Face scope.
