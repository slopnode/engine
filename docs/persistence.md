...
Package UI or gameplay calls save helpers, then (save-write rel form). Loading typically (save-read rel), stashes the form, and (request-map-load map "load"). After the map spawns, (on-map-ready map-id reason) runs; packages treat "load" as restore and "fresh" (the default) as a new run. A common envelope has package-defined body fields:
