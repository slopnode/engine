...
slopfac builds the visible face fragment list static.fac. Requires a sealed [BSP](bsp.md). Authoring stays on [Maps](maps.md). Lightmaps consume FAC face ids: [Radiosity](rad.md).

CMake target slopfac (root CMakeLists.txt), linked against sloplib.

```bash
cmake --build build --target slopfac
...
Re-run after BSP changes that affect sealing or face layout, detail brush edits that change visible fragments, face-id churn, or authored (nodraw). Re-bake [radiosity](rad.md) when chart keys (FAC ids) change.

## What FAC is for

static.fac is the **visible face fragment list** used for the draw mesh, lightmap charts, and Steam Audio occlusion. FAC is the sealed, occluded, welded face fragment list. Leaf/cluster PVS is a separate step: [VIS](vis.md). Runtime object filtering uses the camera frustum and PVS: [View frustum culling](frustum.md).
...
Entry point: tools/slopfac/main.cpp. Core build: buildVisibleFaces in src/map/fac_build.cpp. On-disk format: FAC1 via src/map/fac_io.cpp.

1. Parse CLI (AppConfig); require --map.
2. Require readable static.bsp; load CSG brushes.
3. analyzeMapHull. If not sealed: log leak path, exit 1 (no .fac written).
4. buildVisibleFaces -> clip, weld, cull, merge, sort.
5. Write sibling static.fac with writeFacFile; log face and inferred-nodraw counts.
...
Magic FAC1 (0x31434146), version 2 (reader also accepts v1). Field layout: [Binary formats — FAC1](binary-formats.md#fac1-fac).

## Source map

| Concern | Location |
|---------|----------|
| Visible faces + weld/merge | src/map/fac.hpp, src/map/fac_build.cpp |
| FAC file IO | src/map/fac_io.cpp |
| CLI | tools/slopfac/main.cpp |
| CMake target | root CMakeLists.txt (slopfac) |
